...
public class Nby3RepeatNumber
{
	...
	private boolean isFreqGreaterThanNBy3(final List<Integer> a, Integer key, double nBy3)
	{
		int count = 0;
		for (Integer curInt : a)
		{
			if(curInt.equals(key))
			{
				count++;
				if(count>nBy3)
					return true;
	...
	public int repeatedNumber(final List<Integer> a)
	{
		...
		int aLength = a.size();
		HashMap<Integer, Integer> countMap = new HashMap<Integer, Integer>(K);
		
		for(int i=0; i<aLength; i++)
		{
			Integer curInt = a.get(i);
			if(countMap.containsKey(curInt))
			{
				countMap.put(curInt, countMap.get(curInt)+1);
			}
		...
		double nBy3 = aLength/3.0;
		
		Set<Integer> keySet = countMap.keySet();
		for (Integer key : keySet)
		{
			if(isFreqGreaterThanNBy3(a, key, nBy3))
				return key;
		}
