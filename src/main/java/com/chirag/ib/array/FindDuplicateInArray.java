...
public class FindDuplicateInArray
{
	public int repeatedNumber(final List<Integer> a)
	{
		...
		int listSize = a.size();
		Integer[] aArr = new Integer[a.size()];
		a.toArray(aArr);
		
		for(int i=0; i<listSize; i++)
		{
			int curNum = Math.abs(aArr[i]);
			Integer dataAtIdx = aArr[curNum];
			
			if(dataAtIdx<0)
				return curNum;
			else
				aArr[curNum] = -aArr[curNum];
		}
